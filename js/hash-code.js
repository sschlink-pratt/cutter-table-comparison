...
function getHashCode() {
    const authorName = cleanName(document.getElementById('author').value);
    ...
    if (authorName.length == 0){
        hashCodeEl.innerHTML = "";
        return ''; 
    }
    if (authorName.length == 1) {
        hashCodeEl.innerHTML = authorName[0];
        return '';
    }
    if (authorName.length == 2) {
        hashCodeEl.innerHTML = authorName[0] + authorName[1];
        return '';
    }
    let authorNameToHash = authorName.slice(2);
    // It might be interesting to convert the hash output to octal and then 
    // add 2 to avoid 0 and 1s!
    let hashCode = authorName[0] + authorName[1] + MurmurHash3(authorNameToHash).result();
